...
import { task } from "./bot/cron/mainCron.js";
...
bot.onText(/\/testNoti/, async (msg) => {
  createNotification();
});
bot.onText(/\/testCron/, async (msg) => {
  cronUserUpdate();
});
    ...
    if (!sub) {
      switch (query.data) {
        ...
        case "buy_sng":
          await bot.sendMessage(
            ...
            "💰 Купите подписку, чтобы получить доступ к каналу! подтверждение производится в ручном режиме и может занять некоторое время месяц 10р комментарий к платежу ваш ID",
            {
              reply_markup: {
                inline_keyboard: [
                  [{ text: "Я оплатил", callback_data: "card_paid" }],
                ],
              },
            }
          );
                ...
                reply_markup: {
                  inline_keyboard: [
                    [
                      {
                        text: "1 мес",
                        callback_data: `Adm_subEnablePay_${process.env.OneMonth}_${query.message.chat.id}`,
                      },
                      {
                        text: "2 мес",
                        callback_data: `Adm_subEnablePay_${process.env.TwoMonth}_${query.message.chat.id}`,
                      },
                      {
                        text: "3 мес",
                        callback_data: `Adm_subEnablePay_${process.env.ThreeMonth}_${query.message.chat.id}`,
                      },
                    ],
                    [
                      {
                        text: "6 мес",
                        callback_data: `Adm_subEnablePay_${process.env.SixMonth}_${query.message.chat.id}`,
                      },
                      {
                        text: "12 мес",
                        callback_data: `Adm_subEnablePay_${process.env.TwelveMonth}_${query.message.chat.id}`,
                      },
                    ],
                    [
                      {
                        text: "Отказать",
                        callback_data: `Adm_subDisable_${query.message.chat.id}`,
                      },
                    ],
                  ],
                },
        ...
        case query.data.match(/^Adm_subDisable_*/)?.input: {
          ...
          const { msgId, userId } = await database.approvePreUser(
            queryTelegramIDdis
          );
